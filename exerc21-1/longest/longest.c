...
// Returns only -1, 0 or 1 depending the comparison
...
long int longest_subarray(long int number_of_days, int number_of_hospitals, long int *days) {
    ...
    for (i = 0; i < number_of_days; i++) {
        ...
        // If sum is greater than number_of_hospitals, then answer is i+1.
        if (sum > number_of_hospitals) {
            maxlen = i + 1;
        }

        // If the sum is less than or equal to number_of_hospitals, then
        // find if there is a prefix array having sum
        // that needs to be added to the current sum to
        // make its value greater than number_of_hospitals. If yes, then
